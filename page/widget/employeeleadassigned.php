...
class page_widget_employeeleadassigned extends \xepan\base\Page{
	function init(){
		...
		$grid->js('click')->_selector('.do-view-employee')->univ()->frameURL('Leads Assigned',[$this->api->url('xepan_marketing_widget_leadsassigned'),['start_date'=>$start_date,'end_date'=>$end_date],'employee_id'=>$this->js()->_selectorThis()->closest('[data-id]')->data('id')]);
	}
}